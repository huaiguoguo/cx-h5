    ...
    methods: {
        ...
        countDown() {
            if (!this.timer) {
                ...
                if (this.$store.state.user.editCipherEndTimestamp - 60 < currentTimeStamp) {
                    this.$store.commit('user/setEditCipherEndTimestamp', currentTimeStamp + 60)
                }

                this.timer = setInterval(() => {
                    const currentTimeStampBa = Date.parse(new Date()) / 1000
                    const text = this.$store.state.user.editCipherEndTimestamp - currentTimeStampBa + '秒后重发'
                    if (this.$store.state.user.editCipherEndTimestamp > currentTimeStampBa && this.$store.state.user.editCipherEndTimestamp > 0) {
                        this.secondText = text
                    } else {
                        clearInterval(this.timer)
                        this.timer = null
                        this.secondText = '重新获取'
                        this.$store.commit('user/setEndTimestamp', 0)
                    }
                }, 1000)
            }
        },
